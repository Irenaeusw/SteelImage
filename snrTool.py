...
def framesToVideoAutomated(fps=15, start_num=None, frameNames=None, imgType='.tif'): 
    '''
    Version of function framesToVideo(), but uses tKinter for easy function calling.
    '''

    frames_dir = loadFolderDir("folder containing images") 
    saveDir = loadFolderDir("Save directory for video: ")
    saveDir = saveDir + "\\stitched.avi"
    
    if frameNames == None: 
        frameNames = str(input("Precursor names to frames: ")) 
    if start_num == None: 
        start_num = int(input("Starting number of frames: ")) 
    
    framesToVideo(frames_dir, saveDir, fps=fps, start_num=start_num, fileName=frameNames, imgType=imgType, orderAtlas=False)
...
def opticalFlowVideo(videoPath, saveDir=None, vectorMaps=True, vectorScale=15, fps=5, harrisCorners=400, harrisQuality=0.01, harrisMinDistance=7, harrisBlockSize=7, lkWinSize=(15,15), lkMaxLevel=2, \
    lkCriteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 0.03), dydx=5, strainMapCSV=True, 
    vector_filtering=False, magnitude_threshold=0.1, median_threshold=1.5, 
    grid=False, grid_dimensions=40, initial_offset=5, select_points=False, select_lines=False, stabilizeDiagnostics=False): 
    ...
    February 11 2020 Note: do not use select_points=True parameter, instead use select_lines=True. The wrapping is fcked up and will not work. 
    Args:
        param1: 
    ...
    # Creating grid 
        ...
        # Check if we want to save strain map data into a csv... 

        if strainMapCSV == True: 

            height, width = oldGray.shape 

            # Initialize number of rows and cols 
            grid_rows = int(height/dydx - 1)
            grid_cols = int(width/dydx - 1)

            strainMapDataFolder = createFolder(saveDir, "Strain Map Data")
            strainMap = open("{}\\1.csv".format(strainMapDataFolder), "w")

        p0, strainMapLines = getGridPoints(oldGray, grid_dimensions, dydx, initial_offset, evenGrid=True, wrap_for_cv2=True, strainMapCSV=strainMap)

        strainMap.writelines(strainMapLines)
        strainMap.close() 
    ...
    while(True): 
        ...
        if strainMapCSV == False: 
            ...
            # Append all points disregarding of status is 0 or 1 
            newPoints = new_p1 
            oldPoints = new_p0
...
def getGridPoints(src, grid_dimensions, dydx=5, initial_offset=5, evenGrid=False, wrap_for_cv2=False, strainMapCSV=None): 
    ...
    if evenGrid == False: 
        ...
        # Determine the number of rows and cols in the grid 
        # The offset is function variable dydx...

        # string lines to be written to the csv containing strain map data
        csvLines = [] 

        # initialize rows and columns 
        rows = int(height/dydx - 1)
        cols = int(width/dydx - 1) 
        ...
        if wrap_for_cv2 == False: 
            ...
            for i in range(rows): 
                ...
                # Ensure a new line character is created
                if i != 0: 
                    csvLines[i-1] = csvLines[i-1] + ",\n"

                # initialize a line of data to be written, as well as the first entry, comma separated
                csvLines.append([])
                csvLines[i] = "{}a{},".format(x,y) 
                ...
                for j in range(cols): 
                    ...
                    csvLines[i] = csvLines[i] + "{}a{},".format(x, y) 
                    pointsToTrack[counter].append((x, y)) 
                    counter += 1 
                y += dydx
                x = initial_offset 

    return pointsToTrack, csvLines 
...
# CMS Steel Image 
def analyzePearlites(imageFolder, saveDir): 
    import cv2
    ...
    for contour in contours: 
        ...
        if cv2.curr_area > 100 and cv2.curr_area < 900000:
            pearlites.append(contour) 
            areas.append(cv2.curr_area) 
    ...
    df = pd.DataFrame(data, columns= ["Area (um^2)", "Circular Diameter (um)", "Length (um)", "Widths (um)", "Shape Factors ()"]) 
